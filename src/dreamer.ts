 ...
 */
export function extractDreamProse(markdown: string): string {
  let body = markdown;
  ...
  const proseStart = lines.findIndex(
    (line) => /^#+\s+/.test(line) || /^\*\*[^*]+\*\*$/.test(line.trim())
  );
  if (proseStart > 0) {
    ...
    const cotPattern =
      /^(Now it'?s time|Let me|I need to|I should|I'm going to|Looking at|Based on|ALREADY MAPPED TERRITORY|The \w+ needs?|This (is|was|will))/i;
 ...
 */
export function extractWakingRealization(text: string): string {
  ...
  const cotPatterns =
    /^(This is|Now it'?s time|Let me|I need to|I should|Now I|Looking at|Based on|The dream found|The chain-of-thought)/i;
